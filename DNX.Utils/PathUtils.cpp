...
list<string> PathUtils::GetPathParts(const string& path)
{
    auto parts = StringUtils::SplitTextByAny(
        FixUpPath(path),
        PATH_SEPARATOR + DRIVE_SEPARATOR,
        PATH_SEPARATOR
    );
...
bool PathUtils::IsDriveReference2(const string& filePath)
{
    if (filePath.empty() || filePath.length() > 2)
        return false;

    const auto drive_letter = static_cast<char>(toupper(filePath[0]));
    if (!CharUtils::IsBetween(drive_letter, 'a', 'z') && !CharUtils::IsBetween(drive_letter, 'A', 'Z'))
    {
        return false;
    }
    ...
    {
        const auto drive_separator = to_string(filePath[1]);
        if (drive_separator != DRIVE_SEPARATOR)
        {
            return false;
        }
    }
...
string PathUtils::GetDrive(const string& filePath)
{
    ...
    if (drive_separator_pos == string::npos || drive_separator_pos < 2)
        return "";
